...
const Platos = async ({ searchParams }: SearchParams) => {
  console.log("$$$RENDERIZA Platos", searchParams);
  const page = Number(searchParams?.page ?? 1);
  const query = searchParams?.query ?? "";

  const { entities: platos, total } = await fetchEntities(
    "platos",
    page,
    PAGE_SIZE,
    query,
...
"Disponibilidad",
  ...
  const handleOnDelete = async (id: number) => {
    "use server";
    await deleteEntity("platos", id);
  };

  const handleOnUpdate = async (id: number) => {
    "use server";
    redirect(`/dashboard/platos/${id}/edit`);
  };
  ...
  return (
    <div>
      <div className="flex justify-between p-2 mb-2 bg-gray-200 rounded-md">
        ...
        <CreateButton href="/dashboard/platos" />
      </div>
      <div>
        <AplTable
          entities={platos}
